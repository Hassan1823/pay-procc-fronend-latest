...
const CompanyPresentation = ({ docType, inputId }) => {
  const [file, setFile] = useState(null);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fileName = localStorage.getItem(`CompanyFileName${inputId}`);
    const fileData = localStorage.getItem(`CompanyFileData${inputId}`);
  ...
  const saveFile = (file) => {
    ...
    try {
      localStorage.setItem(`CompanyFileName${inputId}`, file.name);
      const reader = new FileReader();
      reader.onload = function (e) {
        console.log("reader.onload called");
        localStorage.setItem(`CompanyFileData${inputId}`, e.target.result);
      };
  ...
  const removeFile = () => {
    ...
    setFile(null);
    localStorage.removeItem(`CompanyFileName${inputId}`);
    localStorage.removeItem(`CompanyFileData${inputId}`);
  };
