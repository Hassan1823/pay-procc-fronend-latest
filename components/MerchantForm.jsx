...
const MerchantForm = ({ docType, inputId }) => {
  const [file, setFile] = useState(null);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fileName = localStorage.getItem(`MerchnatFileName${inputId}`);
    const fileData = localStorage.getItem(`MerchnatData${inputId}`);
  ...
  const saveFile = (file) => {
    ...
    try {
      localStorage.setItem(`MerchnatFileName${inputId}`, file.name);
      const reader = new FileReader();
      reader.onload = function (e) {
        console.log("reader.onload called");
        localStorage.setItem(`MerchnatData${inputId}`, e.target.result);
      };
  ...
  const removeFile = () => {
    ...
    setFile(null);
    localStorage.removeItem(`MerchnatFileName${inputId}`);
    localStorage.removeItem(`MerchnatData${inputId}`);
  };
