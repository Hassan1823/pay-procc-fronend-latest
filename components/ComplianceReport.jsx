...
const ComplianceReport = ({ docType, inputId }) => {
  const [file, setFile] = useState(null);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fileName = localStorage.getItem(`ReportFileName${inputId}`);
    const fileData = localStorage.getItem(`ReportFileData${inputId}`);
  ...
  const saveFile = (file) => {
    ...
    try {
      localStorage.setItem(`ReportFileName${inputId}`, file.name);
      const reader = new FileReader();
      reader.onload = function (e) {
        console.log("reader.onload called");
        localStorage.setItem(`ReportFileData${inputId}`, e.target.result);
      };
  ...
  const removeFile = () => {
    ...
    setFile(null);
    localStorage.removeItem(`ReportFileName${inputId}`);
    localStorage.removeItem(`ReportFileData${inputId}`);
  };
